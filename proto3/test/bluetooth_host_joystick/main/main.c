...
void hidh_callback(void *handler_args, esp_event_base_t base, int32_t id, void *event_data)
{
    ...
    case ESP_HIDH_INPUT_EVENT: {
		...
		int64_t start_usec = esp_timer_get_time();
		printf("%d.%06d " ESP_BD_ADDR_STR " INPUT:",
				(int)(start_usec / 1000000), (int)(start_usec % 1000000), ESP_BD_ADDR_HEX(bda));
		for (int i = 0; i < param->input.length; i++) {
			printf(" %02x", param->input.data[i]);
		}
		printf(" (");
		for (int i = 0; i < param->input.length; i++) {
			printf(" %d", (int)((int8_t)(param->input.data[i])));
		}
		printf(" )\n");
        break;
    }
