...
static esp_hid_scan_result_t *hid_scan_results = NULL;
static size_t num_hid_scan_results = 0;

static esp_hid_scan_result_t *non_hid_scan_results = NULL;
static size_t num_non_hid_scan_results = 0;
...
#if CONFIG_BT_HID_HOST_ENABLED
static void add_bt_scan_result(esp_bd_addr_t bda, esp_bt_cod_t *cod, esp_bt_uuid_t *uuid, uint8_t *name, uint8_t name_len, int rssi, bool is_hid)
{
    esp_hid_scan_result_t *r = find_scan_result(bda, hid_scan_results);
	if (r == NULL) {
		r = find_scan_result(bda, non_hid_scan_results);
	}
    if (r) {
	...
	if (is_hid) {
		r->next = hid_scan_results;
		hid_scan_results = r;
		num_hid_scan_results++;
	} else {
		r->next = non_hid_scan_results;
		non_hid_scan_results = r;
		num_non_hid_scan_results++;
	}
}
#endif

#if CONFIG_BT_BLE_ENABLED
static void add_ble_scan_result(esp_bd_addr_t bda, esp_ble_addr_type_t addr_type, uint16_t appearance, uint8_t *name, uint8_t name_len, int rssi, bool is_hid)
{
    if (find_scan_result(bda, hid_scan_results)) {
        return;
    }
    if (find_scan_result(bda, non_hid_scan_results)) {
        return;
    }
...
static void handle_bt_device_result(struct disc_res_param *disc_res)
{
	...
	{
        add_bt_scan_result(disc_res->bda, cod, &uuid, name, name_len, rssi, (cod->major == ESP_BT_COD_MAJOR_DEV_PERIPHERAL));
    }
}
#endif
...
static void handle_ble_device_result(struct ble_scan_result_evt_param *scan_rst)
{
    ...
    if (find_scan_result(scan_rst->bda, hid_scan_results)) {
        return;
    }
    if (find_scan_result(scan_rst->bda, non_hid_scan_results)) {
        return;
    }
	...
	{
        add_ble_scan_result(scan_rst->bda, scan_rst->ble_addr_type, appearance, adv_name, adv_name_len, scan_rst->rssi, (uuid == ESP_GATT_UUID_HID_SVC));
    }
}
#endif /* CONFIG_BT_BLE_ENABLED */
...
esp_err_t esp_hid_scan(uint32_t seconds, size_t *num_results, esp_hid_scan_result_t **results)
{
    ...
    *num_results = num_hid_scan_results;
    *results = hid_scan_results;
